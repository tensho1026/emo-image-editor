...
Original / Edited は、プレビュー下のトグルで切り替えます。一度に表示するのは片方だけです。

## できること

- JPEG / PNG / WebP を 1 枚でも複数枚でもアップロード（選択 / ドロップ）
- 8 プリセット: Tide / Abyss / Summer / Dream / Film / Night / Memory / Fog（同じ写真の見本サムネ付き）
- 加工強度 0–100（初期値 50）
- 詳細調整: 明るさ・コントラスト・シャドウ・ハイライト・彩度・色温度・ブルー・色相・フェード・ヘイズ・ぼかし・ブルーム・粒子・ビネット
- ライトリーク / RGBずらし / 空グラデ / 枠
- 記憶の空気: 二重露光（空・月・街灯）/ 窓・雨粒 / フラッシュ焼け / 赤目・白飛び / 色かぶり（緑・マゼンタ）/ スキャン傷 / 折れ・テープ / 手ブレ / 露出段差。スキャン傷は初期オフ
- 切り抜き（自由・1:1・4:5・9:16・16:9）
- レシピを端末に保存（JSON コピー / 読み込み）
- **編集をリセット** と **ひとつ戻る**
- 書き出し: 原寸 / 1920px / 正方形 / Story
- 複数枚を同じ設定でまとめて加工。加工後サムネで切り替え、ZIP 一括保存
- JPEG / PNG ダウンロード
- プレビューは最大 1920px。保存は原寸（最大 4096px）も選べる
